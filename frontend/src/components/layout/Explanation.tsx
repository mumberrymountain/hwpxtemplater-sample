...
const Explanation: React.FC<ExplanationProps> = ({selectedCard, selectedCardIndex, setSelectedCardIndex}) => {
  ...
  return (
    <div className="w-[95%] shrink-0 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 py-2">
        ...
        <div className="min-w-0 flex-1">
            <h2 className="text-xl md:text-[36px] font-bold mb-2 break-words">
            {selectedCard.detailTitle}
            </h2>
            <h4 className="mt-1 text-[#a8a5a8] font-normal text-sm md:text-[17px] leading-[1.5] break-words">
        ...
        <button 
                ...
                className="bg-[#1a1a1a] text-white border border-[#2a2a2a] rounded-lg px-3 md:px-4 py-2 cursor-pointer text-xs md:text-sm shrink-0 self-start sm:self-center"
        >
            ← 뒤로가기
        </button>
        )}
